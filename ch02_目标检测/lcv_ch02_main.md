...
第二步，将映射后的ROI区域划分为与输出维度相同的切片, 比如输入特征图是 $5 \times 7$, 想要一个 $2 \times 2 $ 的输出；

第三步，对每个切片进行最大池化操作。

这样就可以从不同尺寸的ROI提议区域得到固定大小的特征图，该ROI Pooling的结果特征图不依赖ROI的尺寸。

例如：输出特征图的尺寸为：$8 \times 8$，提议区域ROI的左上角和右下角坐标分别为（0,100）和（198,224），规定输出大小为：$2 \times 2$。
...
第二步，将ROI划分成$2 \times 2$的区域，产生了量化误差。

映射到特征图后ROI的宽为7，划分2份后，每个区域的宽为：7/2 = 3.5，部分区域的宽取3，部分区域的宽取4

映射到特征图后ROI的长为5，划分2份后，每个区域的宽为：5/2 = 2.5，部分区域的宽取2，部分区域的宽取3
